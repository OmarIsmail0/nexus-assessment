...
import type { GetMoviesParams, MovieDetailsResponse, MovieSearchResponse } from "../../types";
 ...
 */
export const getMovies = async ({ imdbID, title, type, year, page }: GetMoviesParams): Promise<MovieDetailsResponse | MovieSearchResponse> => {
    const response = await agent.get<MovieDetailsResponse | MovieSearchResponse>(``, {
        params: {
            apikey: API_KEY,
            s: title || undefined,
            i: imdbID || undefined,
 ...
 * @returns {Promise<Object>} The response data from the OMDb API.
 */
export const getMoviePoster = async (imdbID: string): Promise<any> => {
